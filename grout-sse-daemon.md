...
- DONE: Filter sessions whose id starts with the literal prefix "agent-" out of what grout emits or the plugin renders (implementation's call which layer) -- this replaces reliance on --include-automated/--include-children
  Commit e3619bb (grout layer, same single-point-of-enforcement place cycle 2 used). Red: `list_test.go:51:9: undefined: excludeSubagents` (compile fail, both new tests). Green after adding `excludeSubagents` (prefix check) and wiring it into `listSessions` right after `decodeSessionList`. No plugin change needed or made — grout never emits the row, so `sessions_in_own_tab` never sees a subagent session; `cargo test` 133/133 unaffected, confirming the choice of layer added no Rust surface.
- DONE: Verify against real, live agentsview data, not only a synthetic/fixture session; fixtures seeded with the real id shape
  Queried the already-running local daemon (`agentsview serve status`, pid 66006) via `session list --server http://127.0.0.1:8080 --json --include-one-shot --include-children --limit 500`: 412 real `agent-`-prefixed subagent ids among 500 real sessions (e.g. `agent-aspacedock-ensign-grout-sse-daemon-implementation-2ecb50dc015c0ece`, `agent-a53bbd2c12c218b7a`). Ran the exact `excludeSubagents` prefix rule against that real captured envelope in a throwaway script: 500 in, 88 out, 0 agent-prefixed remaining, real UUID ids kept unchanged (`82a5da69-…`, `38b9fa21-…`). Test fixtures (`TestExcludeSubagents`, `TestListSessionsFiltersSubagents`) seed one real captured id (`agent-a53bbd2c12c218b7a`) rather than an invented placeholder.
- DONE: Update AC-6's baseline to state the filter as "id does not start with agent-", not "excluding automated/child sessions". Add or extend a test asserting a session with an agent--prefixed id is filtered even when its cwd matches the tab's own project
  AC-6 baseline rewritten (state checkout commit d2a4344) to the id-prefix phrasing; a new "Subagent-id filter" AC line added with its own Verified-by. `TestExcludeSubagents` and `TestListSessionsFiltersSubagents` (grout commit e3619bb) both give the subagent session the same cwd as the kept session, so a cwd match cannot save it from the filter — the closest Go-layer analogue to "cwd matches the tab's own project" since tab scoping is a plugin-only concept grout never sees.

### Summary

Replaced reliance on agentsview's `--include-automated`/`--include-children` flags (cycle 2, shown by CL's live observation not to work) with an id-prefix filter: `excludeSubagents` drops any session whose id starts with `agent-` — the shape every Task-tool-spawned subagent's id has — inside `listSessions`, so no subagent row is ever built or emitted. Two red-first Go tests added (`TestExcludeSubagents` on the pure filter, `TestListSessionsFiltersSubagents` end-to-end through the exec path), both seeded with a real captured subagent id rather than an invented one. Verified against real, live data beyond the fixtures: pulled 500 real sessions off the currently-running local agentsview daemon (412 genuinely `agent`-prefixed) and confirmed the exact filter logic drops all 412 and keeps the other 88. No plugin change was needed since grout is the single point of enforcement, matching cycle 2's established pattern (confirmed unaffected: `cargo test` 133/133). Go: 16 → 18 test funcs, `go vet` clean. AC-6 and the design narrative (grout/README.md, entity body) updated to name the real mechanism; AC-1..5, AC-8 untouched.

## Stage Report: validation (cycle 3)

- DONE: Independently re-run AC-1/2/3/5 (unaffected) and the revised AC-6/subagent-id-filter ACs against the worktree's cycle-3 commit, re-executed not re-read
  All PASS, fresh `-count=1` at `153862d`: AC-1 `TestWatchSessionsFlow`, AC-2 `TestWatchAutoStart|TestWatchReconnect`(+silence test), AC-3 `TestWatchWedgedPipe`, AC-4 the 3 named cargo tests (plugin untouched this cycle) + full `cargo test` 133/133 + `cargo check --tests`, AC-5 `go test ./... && go vet ./...`, subagent-id filter `TestExcludeSubagents|TestListSessionsFiltersSubagents`. Pulled real live agentsview data myself (not the implementer's 500-session capture): ~2,500 sessions across 3 independent time slices, cross-checked id-prefix against the independent `relationship_type`/`parent_session_id` ground-truth field. 0 false positives; 0 false negatives for `claude`. Full table in `docs/agent-rail-dev/.spacedock-state/gates/grout-sse-daemon-validation.md` under "Cycle 3".
- DONE: Refutation audit on a throwaway checkout (never the implementation worktree) targeting the agent- prefix filter specifically: constructed/found a false positive and a false negative
  Fresh `git clone` + `checkout 153862d` (discarded clean after). No false positive found in ~2,500 real sessions or by construction (prefix is generated by one internal code path). Real false negative found and proven against the shipped code: Codex's own subagent mechanism produces `codex:<uuid>` ids for genuine `relationship_type: subagent` children (e.g. `codex:019d4f0d-cf13-7480-ac25-295491b7a9a7`, a real spacedock worker) — a throwaway `TestRefutation_CodexSubagentIDShapeSurvivesFilter` confirms `excludeSubagents` does not drop it. Currently non-blocking: `listSessions` never passes `--include-children`, and agentsview's own default independently confirmed to already exclude all `relationship_type: subagent` sessions server-side today (codex 807→198, claude 19348→2408 with the flag dropped) — but this is the same kind of classifier cycle 2 already caught failing once for Claude, and CL actively dispatches Codex workers, so flagged as a named residual risk rather than buried in a PASS. Nested sub-subagent search (0 found in ~1,000 recent sessions) reported as inconclusive, not proven either way. Full detail in the gate file's "Cycle 3" section.
- DONE: Prepare the final revised AC-6 demo script for CL's live session (tab-scoped, subagent-excluded count parity, unattended new-session appearance) and confirm no cross-tab step remains anywhere in it
  Written to the gate file's "Cycle 3" section: baseline now computed via the actual id-prefix mechanism (superset query + `jq` `startswith("agent-")` filter) rather than trusting agentsview's own default, specifically so the demo can catch a regression in grout's own filter rather than accidentally re-testing the upstream layer; added a new step dispatching a real Task-tool subagent inside the demo tab's own project to confirm a cwd match doesn't save it (the crux of this cycle's fix); confirmed zero cross-tab steps (AC-7 stays superseded by AC-4). Corrected a stale carry-over: this cycle's diff is grout-only (`git diff --stat 8b04a4f..153862d`), so unlike cycles 1/2 no plugin/wasm rebuild is required.

### Summary

Independently re-ran every offline AC (AC-1..5, subagent-id filter) fresh against `153862d` — all PASS. Pulled my own live agentsview data (~2,500 sessions, 3 time slices, versions spanning 2.1.15–2.1.25 plus codex/pi/cowork/gemini) rather than trusting the implementer's sample, and cross-checked the id-prefix rule against the independent `relationship_type` ground-truth field: 0 false positives, 0 false negatives for Claude. Found one real false negative class — Codex-spawned subagents use `codex:` ids, never `agent-`-prefixed — proved against the shipped `excludeSubagents` on a throwaway checkout with a real captured record. It's non-blocking today only because agentsview's own default independently confirmed to already exclude these server-side, the same kind of layer cycle 2 showed can silently fail; named explicitly since CL actively runs Codex workers. Wrote the final AC-6 demo script with a baseline that exercises the real id-prefix mechanism instead of the masking default layer, added a live subagent-cwd-collision step, and confirmed no cross-tab step remains.
